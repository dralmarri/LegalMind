#!/bin/bash
# أمر آلي 82: إلحاق مراجع مبادئ الكتاب الخامس عشر — حوكمة الشركات (77 مرجعاً منسوخاً بصرياً) بمبادئها في القاعدة
set -e
set -a; source /opt/LegalMind/deploy/.env; set +a
PY=/opt/LegalMind/.venv/bin/python
curl -fsSL "https://raw.githubusercontent.com/dralmarri/LegalMind/af43a2e82ca7479e2feb219c170af842e1c34202/ops/autorun/refs-book15.json" -o /tmp/refs-book15.json
...
entries = json.load(open('/tmp/refs-book15.json', encoding='utf-8'))
for e in entries:
    e['sig'] = sigset(e['k'])
...
with eng.psycopg.connect(eng.database_url()) as conn:
    ...
    objs = cur.fetchall()
    print("مبادئ الكتاب الخامس عشر في القاعدة:", len(objs), flush=True)
