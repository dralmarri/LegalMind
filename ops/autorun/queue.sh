#!/bin/bash
# أمر آلي 84: إلحاق مراجع مبادئ الكتاب العاشر — الإفصاح والشفافية (128 مرجعاً منسوخاً بصرياً) بمبادئها في القاعدة
set -e
set -a; source /opt/LegalMind/deploy/.env; set +a
PY=/opt/LegalMind/.venv/bin/python
curl -fsSL "https://raw.githubusercontent.com/dralmarri/LegalMind/dc6701a16d9fb91030b4704d4c748495db7ba422/ops/autorun/refs-book10.json" -o /tmp/refs-book10.json
...
entries = json.load(open('/tmp/refs-book10.json', encoding='utf-8'))
for e in entries:
    e['sig'] = sigset(e['k'])
...
with eng.psycopg.connect(eng.database_url()) as conn:
    ...
    cur.execute("""SELECT id, coalesce(title,''), coalesce(original_text,'') FROM knowledge_objects
                   WHERE object_type='judicial_principle'
                     AND metadata->>'title' LIKE '%%الكتاب ١٠%%'""")
    objs = cur.fetchall()
    print("مبادئ الكتاب العاشر في القاعدة:", len(objs), flush=True)
