...
**المنهجية (إفصاح صريح عن النطاق):** قياس حقيقي (instrumentation only، بلا أي تغيير سلوكي —
انظر §6) على **عيّنة ممثِّلة من 6 حالات** (`gs-0013`، `gs-0036` — حالتا الفجوة الفعلية؛ +
`gs-0002`، `gs-0018`، `gs-0020`، `gs-0035` — عادية متنوعة الفئة) عبر Shadow A **و**Shadow B
لكل حالة (12 نداء `_draft_build_context` حقيقي إجمالًا) — **لا الأربعين كلها**، تخفيفًا لكلفة
شبكة/Haiku متراكمة كبيرة من جولات القياس السابقة في نفس الجلسة. الأرقام أدناه تقديرات على
عيّنة صغيرة، لا معدلات إنتاج نهائية.

### 5.1 Runtime (بالمللي ثانية، عبر التجميع الفعلي لكل تسمية)

| المرحلة | العدد | p50 | p95 | max | mean |
|---|---:|---:|---:|---:|---:|
| **current_pipeline_runtime_ms** (الأساس الحقيقي، `_draft_build_context` كاملة) | 12 | 38,100 | 64,144 | 73,001 | **45,205** |
| **reranker** (المرتِّب المتقاطع) | 12 | 25,188 | 47,201 | 48,707 | **28,708** |
| embedding (تضمين متعدد الاستعلامات) | 13 | 9,457 | 10,754 | 40,849* | 11,920 |
| haiku_subqueries (تفكيك المحاور) | 13 | 1,928 | 2,565 | 2,643 | 2,049 |
| lexical_query | 13 | 1,163 | 1,887 | 1,963 | 1,338 |
| qdrant_search | 186 | 21 | 189 | 1,026 | 56 |
| pg_fetch_texts | 59 | 22 | 35 | 58 | 23 |
| direct_ids_query | 25 | 0.1 | 151 | 285 | 38 |
| bundle_query | 13 | 12 | 40 | 114 | 23 |
| chapter_query | 25 | 12 | 24 | 165 | 19 |
| **backbone_ms** (Backbone الحتمي — كود P2.1) | 12 | 12.7 | 131.4 | 137.4 | **32.8** |
| **admission_ms** (الحجز/القبول — كود P2.1) | 12 | 0.1 | 0.1 | 0.1 | **0.1** |
| **coverage_checker_ms** (فحص التغطية — كود P2.1) | 12 | 0.1 | 0.1 | 0.1 | **0.1** |
| **shadow_a_total_ms** | 6 | 38,261 | 73,030 | 73,030 | **48,411** |
| **shadow_b_total_ms** | 6 | 36,355 | 61,657 | 61,657 | **44,439** |

*\* قيمة `embedding` القصوى (40.8 ثانية) شذوذ منفرد يتوافق مع نمط توثَّق تاريخيًا في المشروع
(تحميل نموذج التضمين من القرص عند أول نداء في العملية) — لا مؤشر تدهور مستمر.*

### 5.2 القراءة الحاسمة: كلفة P2.1 نفسها مهمَلة عمليًا

**مجموع (backbone_ms + admission_ms + coverage_checker_ms) ≈ 33 مللي ثانية، مقابل ≈ 45,205
مللي ثانية لخط الأساس الحقيقي — أي 0.07% فقط من الزمن الكلي.** الفارق بين `shadow_a_total_ms`
(48,411) وخط الأساس (45,205) يقع بالكامل تقريبًا ضمن التذبذب الطبيعي بين التشغيلات الحية
المستقلة، لا كلفة إضافية حقيقية لمنطق P2.1. **الكلفة الحقيقية بالكامل تقريبًا (>95%) تعود
للمرتِّب المتقاطع (reranker، ~28.7 ثانية وسطيًا) والتضمين (~11.9 ثانية) — بنية الإنتاج
القائمة أصلًا، لا أي إضافة من هذه الجولة.**

### 5.3 Retrieval / Embedding / Reranker Calls (لكل الحالات الست معًا، Shadow A+B مجتمعتين)
```
Qdrant calls (qdrant_search)     = 186
PostgreSQL calls (pg_fetch_texts) = 59
Embedding calls                   = 13
Haiku (تفكيك محاور) calls          = 13
Reranker calls                    = 12
Bundle/Direct/Chapter/Lexical     = 13 / 25 / 25 / 13
```

### 5.4 Context Size Delta
مُغطًّى في §2.3 (Shadow A: +1453 حرفًا) و§3.5 (Shadow B: +617 حرفًا) — إجمالي +2070 حرفًا صافيًا
عبر الأربعين حالة كاملة (لا العيّنة الست فقط، هذا الرقم من التشغيلة الرئيسية الكاملة).
...
5. **قياس التكلفة (§5) مبني على عيّنة 6 حالات فقط، لا الأربعين كلها** — إفصاح نطاق متعمَّد لا فجوة مخفية؛ الاتجاه العام (كلفة P2.1 نفسها مهمَلة، المرتِّب المتقاطع هو المهيمن) واضح لكن الأرقام المطلقة تقديرية.
...
| Runtime Cost | **PASS** — كلفة منطق P2.1 نفسه (backbone+admission+coverage) ≈0.07% من الزمن الكلي (~33ms من ~45,205ms)؛ الكلفة الفعلية بالكامل تقريبًا (>95%) تعود للمرتِّب المتقاطع والتضمين، بنية إنتاج قائمة أصلًا لا إضافة من P2.1 (قياس على عيّنة 6 حالات، لا الأربعين) |
