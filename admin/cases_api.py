...
def configure_case_router(auth_dependency):
...
def require_configured_auth():
    if _auth_dependency is None:
        raise RuntimeError("Case router authentication is not configured")
...
@router.get("")
def list_cases(_: Annotated[str, require_configured_auth()]):
    return db_fetch(
...
@router.post("")
def create_case(payload: CaseCreate, _: Annotated[str, require_configured_auth()]):
    row = db_execute(
...
@router.get("/{case_id}")
def get_case(case_id: str, _: Annotated[str, require_configured_auth()]):
...
def update_case(case_id: str, payload: CaseUpdate, _: Annotated[str, require_configured_auth()]):
    data = payload.model_dump(exclude_none=True)
    if not data:
        return get_case(case_id, _)
    ...
    params = tuple(data.values()) + (case_id,)
    row = db_execute(f"UPDATE legal_cases SET {assignments},updated_at=now() WHERE id=%s RETURNING *", params)
    if not row:
        raise HTTPException(404, "القضية غير موجودة")
    return row
...
def case_coverage(case_id: str, _: Annotated[str, require_configured_auth()]):
    ...
    counts = db_fetch(
        ...
        (case["branch"], case.get("topic") or "", case.get("topic") or "", case.get("subtopic") or "", case.get("subtopic") or ""),
    )
    ...
    judgments = by_type.get("full_judgment", 0)
    ...
    return {
        ...
        "note": "وجود حكم كامل يعزز جودة التطبيق لكنه ليس شرطًا آليًا وحيدًا لفتح المسودة الأولية."
    }
