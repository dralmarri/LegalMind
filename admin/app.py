...
10. عند صياغة صحيفة دعوى أو مذكرة دفاع أو مذكرة تعقيب أو أي مستند قضائي، استعمل — حيث يلائم السياق طبيعيًا ودون تكلّف أو حشو أو تكرار مفرط — أسلوب اللغة القانونية الكويتية المتَّبع فعليًا في المحاكم:
   - افتتاح الصحيفة: «حاصل وقائع الدعوى أنه بتاريخ...» / «تتلخص وقائع الدعوى أنه بتاريخ...» / «تتمثل حيثيات الدعوى أنه بتاريخ...».
   - روابط بين الجمل (تصلح لكل الفروع): «من المعلوم في لغة القانون أن»، «من المستقر عليه قانونًا وقضاءً وفقهًا أن»، «يدرك أهل الاختصاص القانوني أن»، «وهو ما حدا به»، «وذلك على سند من القول»، «ولما كان الثابت بالأوراق»، «ومفاد هذا النص ولازمه»، «ومفاد النصوص سالفة البيان»، «وبهذا يكون المشرع قد سدّ»، «وترتيبًا على ما تقدم»، «وإزاء ما تقدم»، «واهتداءً بما تقدم»، «ونزولًا على ما سلف بيانه»، «تأسيسًا على ما سبق وعلى ضوء القضاء الذي أرسته المحكمة العليا واستنادًا على البيّن من مبادئ الشريعة الغراء»، «وبتطبيق ما تقدم من نصوص على الواقعة المعروضة يظهر جليًا»، «وردًا على ذلك»، «علاوة على ذلك»، «فضلًا عن ذلك»، «والقول بعكس ما تقدم مؤداه».
   - عبارات الترجيح والرد على حجج الخصم (تصلح لكل الفروع غير الجزائية وللجزائي معًا إلا ما استُثني أدناه): «ولا ينال من ذلك ما دفع به»، «ولا ينهض ذلك ما أورده»، «الأمر الذي لا مراء فيه أن... منوط»، «ذلك أن حقيقة المسألة المعروضة تكمن في...وليس ما أورده المدعي»، «وغاية ما في الأمر»، «وإزاء هذا التعارض الصريح»، «ولما كان البين من استقراء أوراق الدعوى»، «ولما لم يحز قبول هذا الحكم»، «فلا يسوغ له إجهاض النص القانوني»، «وفق صحيح القانون»، «وبإلباس الوقائع الوصف القانوني السليم يستبين»، «وبإسقاط الحكم القانوني الوارد في المادة السابق بيانه انطباقها على الحالة المعروضة، حيث...»، «وإن كان سرد الوقائع في مجمله سديدًا إلا أنه أغفل التعريج على...»، «أما ما ذهب إليه المدعي من قول... فقد جانبه الصواب في ذلك»، «ولا يشفع للمدعي إضافته لمحسنات لفظية من أجل... فهذا لا يعدو سوى استعطاف غير مبرر هدفه الميل عن تطبيق صحيح القانون».
   - عبارات جزائية الطابع حصرًا (تستعمل فقط في مسائل جزائية فعلية، ويُحظر استعمالها في مدني أو تجاري أو إداري أو أحوال شخصية): «كما أن المجني عليها ولما تخلقت عليها المنافذ القانونية لجأت إلى التعويل على الكلام المرسل الذي لا يثبت أيًا من أركان الجريمة»، «بتمحيص أوراق الدعوى وما حوته من مستندات يستبين عدم محورة الاتهام على سند مقبول قانونًا»، «بتمحيص الأدلة المعروضة يتضح محاولة المدعي مرارًا وتكرارًا لتسوية الوضع مع المدعى عليها وديًا إلا أن المدعى عليها قابلت الأمر بالمماطلة والتسويف دون ثمة مبرر أو مسوغ قانوني».
...
def _gap_confirmed_art(window_text):
    ...
    أُضيف 2026-09-07 بعد رصد حي: إفصاح صادق كـ«المادة 166 لم يرد نصها في السياق» كان
    يُعامَل بثقة أضعف من استشهاد جريء غير مسند بنفس الرقم (الذي يجلبه _audit_answer
    مؤكَّدًا) — فالإفصاح الصادق برقم صريح يستحق تحقق القاعدة المباشر لا التخمين الدلالي.
    يُطبَّع النص أولًا (ة←ه) بنفس تطبيع _audit_answer — بلا هذا التطبيع لا يطابق
    _AUD_ART_RE «المادة» الخام (يطابق «الماده» فقط بعد التطبيع)، كما كشف اختبار محلي."""
    window_text = _expand_art_lists(_draft_norm_ar(window_text or ""))
    spans = []
    for _key, _p in _LAW_ALIASES_TBL:
        i = window_text.find(_key)
        while i >= 0:
            spans.append((i, _p))
            i = window_text.find(_key, i + 1)
    for r in _LAWREF_RE.finditer(window_text):
        pre = window_text[max(0, r.start() - 30): r.start()]
        if any(k in pre for k in ("طعن", "دعو", "توكيل", "سند", "قضي", "قبض",
                                  "محضر", "جلس", "تاريخ", "مورخ", "حافظ")):
            continue
        if pre[-1:] in "/0123456789":
        ...
        a, b, c, d, e, f = r.groups()
        n, y = (a, b) if a else (c, d) if c else (e, f)
        spans.append((r.start(), "legis-" + n.translate(_DIG) + "-" + y.translate(_DIG) + "-"))
    if not spans:
        return None
    best, bestkey = None, None
    for m in _AUD_ART_RE.finditer(window_text):
        pos = m.start()
        for s, p in spans:
            d = s - pos
            if abs(d) > 90:
            ...
            key = (0 if d >= 0 else 1, abs(d))
            if bestkey is None or key < bestkey:
                bestkey, best = key, p + "m" + m.group(1)
    return best


def _gap_search(answer, seen):
    # عند اعتراف المسودة الأولى بفجوة استرجاع صريحة («... لم يرد ... في السياق ...»)،
    # مساران بحسب صراحة الفجوة (أُضيف المسار الأول 2026-09-07): (1) رقم مادة صريح في
    # نافذتها ← تحقق مباشر من القاعدة وجلب مؤكَّد (نفس ثقة _audit_answer)؛ (2) غير ذلك
    # ← بحث دلالي مستهدف واحد عن موضوع الجملة المحيطة (سقف فجوتين، أعلى مرشحين لكل
    # فجوة) عبر قناتي التضمين وQdrant الحيتين نفسيهما — لا يمس مسار الاسترجاع
    ...
    # بالمرتبة الأولى) رغم تقارب درجات المتجه — فلا حكمَ نهائيًا هنا على صلة القناة
    # الدلالية؛ يُترك ذلك للنموذج بإطار صياغي صريح يطلب التحقق قبل الاستشهاد (المرتِّب
    # المتقاطع غير معتمد لهذه الطبقة — رُصد معطَّلًا صامتًا في الإنتاج وقت البناء).
    # أي فشل يعيد قوائم فارغة آمنة ولا يمس الصياغة.
    confirmed, tentative = [], []
    try:
        ...
        for m in matches:
            ...
            cid = _gap_confirmed_art(topic + " " + m.group())
            windows.append((topic, cid))
            if cid and cid not in seen and cid not in cand_ids:
                cand_ids.append(cid)
        if cand_ids:
            dsn = _draft_env("DATABASE_URL")
            if dsn:
                try:
                    import psycopg
                    with psycopg.connect(dsn) as conn, conn.cursor() as cur:
                        cur.execute("SELECT id FROM knowledge_objects WHERE id = ANY(%s)", (cand_ids,))
                        exist = {r[0] for r in cur.fetchall()}
                    confirmed = [i for i in cand_ids if i in exist][:4]
                except Exception:
                    confirmed = []
        for topic, cid in windows:
            if cid in confirmed:
                continue          # رقم صريح مؤكَّد لهذه الفجوة — لا حاجة للتخمين الدلالي
            if len(topic) < 8:
                continue
            ...
            for h in hits:
                ...
                if oid and oid not in seen and oid not in tentative:
                    tentative.append(oid)
                    picked_here += 1
...
def _draft_run(inp: _DraftIn) -> dict:
    ...
    gap_confirmed = []
    try:
        gap_confirmed, gap_ids = _gap_search(answer, seen)
    except Exception as _ge:
        print("[draft] gap-search-error:", repr(_ge), flush=True)
    if gap_confirmed:
        selffix_ids = list(dict.fromkeys(selffix_ids + gap_confirmed))[:12]
        print("[draft] gap-confirmed:", len(gap_confirmed), gap_confirmed[:10], flush=True)
    if gap_ids:
