#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""مُدخِل القانون رقم 8 لسنة 2010 بشأن حقوق الأشخاص ذوي الإعاقة.
المصدر: DOCX بطبقة نصّ سليمة (استُخرج آليًا ونُظّف وروجع). 72 مادة + (42 مكرر) + ديباجة،
في عشرة فصول. معرّفات: legis-8-2010-m{N} (و legis-8-2010-m42mkr) + legis-8-2010-preamble.
...
LAW_NO   = 8
LAW_YEAR = 2010
LAW_TAG  = "قانون حقوق الأشخاص ذوي الإعاقة (8/2010)"
BRANCH   = "إداري"
INSTRUMENT = "القانون رقم 8 لسنة 2010 بشأن حقوق الأشخاص ذوي الإعاقة"

PREAMBLE = (
    "القانون رقم 8 لسنة 2010 بشأن حقوق الأشخاص ذوي الإعاقة\n"
    "بعد الاطلاع على الدستور، وعلى قانون الجزاء (16/1960)، وقانون الإجراءات والمحاكمات "
    "الجزائية (17/1960)، وقانون العمل في القطاع الأهلي (38/1964)، وقانون العمل في قطاع "
    "الأعمال النفطية (28/1969)، وقانون التأمينات الاجتماعية (61/1976)، وقانون المساعدات "
    "العامة (22/1978)، والمرسوم بالقانون رقم 15 لسنة 1979 في شأن الخدمة المدنية ونظامها، "
    "وغيرها من القوانين ذات الصلة. وافق مجلس الأمة على القانون التالي نصه:"
)
...
def build_rows(data):
    ...
    for key in order:
        ...
        rows.append({
            ...
            "text": a["text"].strip(),
            "meta": meta({"article_number": num, "fasl": fasl})})
    return rows
...
def main():
    ...
    for r in rows:
        if r["meta"].get("chunk") == "preamble":
            continue
        byfasl.setdefault(r["topic"], []).append(r["meta"]["article_number"])
    print("== تقرير قانون حقوق الأشخاص ذوي الإعاقة (8/2010) ==")
