#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""مُدخِل المرسوم بالقانون رقم 23 لسنة 1990 بشأن تنظيم القضاء.
5 مواد إصدار + القانون المرافق (74 مادة + مكرران) في ستة أبواب وفصولها.
معرّفات: legis-23-1990-{key} (issN لمواد الإصدار) + preamble. الفرع «إداري»."""
from __future__ import annotations
import argparse, json, os, re, sys

LAW_ID   = "legis-23-1990"
LAW_TAG  = "قانون تنظيم القضاء (23/1990)"
BRANCH   = "إداري"
INSTRUMENT = "المرسوم بالقانون رقم 23 لسنة 1990 بشأن تنظيم القضاء"

PREAMBLE = (
    "المرسوم بالقانون رقم 23 لسنة 1990 بشأن تنظيم القضاء\n"
    "بعد الاطلاع على الدستور، وعلى المرسوم الأميري رقم 19 لسنة 1959 بقانون تنظيم القضاء، "
    "وقانون الإجراءات الجزائية (17/1960)، وقانون إنشاء محكمة أمن الدولة (26/1969)، وقانون "
    "الطعن بالتمييز (40/1972)، والمرسوم بالقانون رقم 14 لسنة 1977 في شأن درجات ومرتبات "
    "القضاة وأعضاء النيابة العامة وإدارة الفتوى والتشريع، والمرسوم بالقانون 15/1979 في شأن "
    "الخدمة المدنية. أمرنا بالقانون الآتي نصه. ينظّم القانون المحاكم ومجلس القضاء الأعلى "
    "والقضاة والنيابة العامة والعاملين بالمحاكم."
)
...
def build_rows(data):
    ...
    def meta(extra=None):
        m = {"instrument": INSTRUMENT, "instrument_rank": "مرسوم بقانون",
             "law_number": 23, "law_year": 1990, "upload_origin": "docx_clean_textlayer"}
        if extra:
            m.update(extra)
        return m
...
def main():
    ...
    print("== تقرير قانون تنظيم القضاء (23/1990) ==")
