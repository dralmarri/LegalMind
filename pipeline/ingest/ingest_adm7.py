#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""مُدخِل القانون رقم 53 لسنة 2001 في شأن الإدارة العامة للتحقيقات بوزارة الداخلية.
المصدر: DOCX بطبقة نصّ سليمة (استُخرج آليًا ونُظّف وروجع). 27 مادة + ديباجة. الفرع «عمل».
معرّفات: legis-53-2001-m{N} + legis-53-2001-preamble."""
from __future__ import annotations
import argparse, json, os, re, sys

LAW_ID   = "legis-53-2001"
LAW_NO   = 53
LAW_YEAR = 2001
LAW_TAG  = "قانون الإدارة العامة للتحقيقات (53/2001)"
BRANCH   = "إداري"
INSTRUMENT = "القانون رقم 53 لسنة 2001 في شأن الإدارة العامة للتحقيقات بوزارة الداخلية"
TOPIC = "الإدارة العامة للتحقيقات بوزارة الداخلية"

PREAMBLE = (
    "القانون رقم 53 لسنة 2001 في شأن الإدارة العامة للتحقيقات بوزارة الداخلية\n"
    "ينظّم هذا القانون الإدارة العامة للتحقيقات بوزارة الداخلية واختصاصاتها المقررة بالمادة "
    "التاسعة من قانون الإجراءات والمحاكمات الجزائية، وتشكيلها (مدير عام، نواب، مدعون عامون، "
    "رؤساء تحقيق، محققون)، وشروط تعيين أعضائها وترقياتهم وتأديبهم وحصاناتهم وواجباتهم."
)
...
def main():
    ...
    print("== تقرير قانون الإدارة العامة للتحقيقات (53/2001) ==")
