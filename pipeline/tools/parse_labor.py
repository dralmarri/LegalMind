...
json.dump({"articles":articles,"order":order}, open('labor_parsed.json','w',encoding='utf-8'), ensure_ascii=False, indent=1)

# --- post-process: normalize bullets and Arabic-Indic digits in body, tidy ---
import json as _j
data=_j.load(open('labor_parsed.json',encoding='utf-8'))
def tidy(t):
    lines=t.split("\n")
    out=[]
    for ln in lines:
...
for n,a in data["articles"].items():
    a["text"]=tidy(a["text"])
_j.dump(data, open('labor_parsed.json','w',encoding='utf-8'), ensure_ascii=False, indent=1)
print("tidied.")
print("\n=== مادة 1 ===")
