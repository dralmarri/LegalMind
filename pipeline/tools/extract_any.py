import zipfile, re, struct, olefile, sys
...
def from_docx(p):
    ...
    out=[]
    for para in paras:
        t=''.join(re.findall(r'<w:t[^>]*>(.*?)</w:t>', para, re.S)).strip()
        if t: out.append(t)
    return '\n'.join(out)
...
def from_doc(p):
    ...
    n=(len(pcd)-4)//12
    cps=[struct.unpack_from('<I',pcd,k*4)[0] for k in range(n+1)]
    out=[]
    for k in range(n):
