...
blocks=[]; cur=None; pend=None; prev_a3=False
for s,t in body_rows:
    num=hdr_num(t)
    if num is not None:
        cur={'num':num,'crumb':dict(crumb),'body':[],'kind':'article'}; blocks.append(cur); pend=None; prev_a3=False; continue
    if s=='a3':
        if t!='وفيه فصول':
            if prev_a3 and crumb['pillar']:
                crumb['pillar']=crumb['pillar']+' — '+t
            else:
                crumb['pillar']=t
        pend=None; cur=None; prev_a3=True; continue
    prev_a3=False
